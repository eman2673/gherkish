...
import { HttpClient, createDefaultApis, sendRequestFactory } from './http.client';
...
describe('HttpClient', () => {
  ...
  describe('API Registration', () => {
    it('should register and retrieve API configurations', () => {
      ...
      httpClient.registerApi('test-api', apiConfig);
  ...
  describe('sendRequest', () => {
    ...
    it('should make successful GET request', async () => {
      ...
      httpClient.registerApi('test', { baseUrl: 'https://api.example.com' });
  ...
  describe('Default APIs', () => {
    it('should create default API configurations', () => {
      // Set the required environment variable for the test
      const originalEnv = process.env.APP__BASE_URL;
      process.env.APP__BASE_URL = 'http://localhost:3000';
      const env = process.env as Record<string, unknown>;

      try {
        createDefaultApis(httpClient);

        const defaultApi = httpClient.getApi('default');
        const authenticatedApi = httpClient.getApi('authenticated');

        expect(defaultApi).toBeDefined();
        expect(authenticatedApi).toBeDefined();
        expect(defaultApi?.baseUrl).toBe('http://localhost:3000');
        expect(authenticatedApi?.baseUrl).toBe('http://localhost:3000');
      } finally {
        // Restore the original environment variable
        if (originalEnv) {
          process.env.APP__BASE_URL = originalEnv;
        } else {
          delete env.APP__BASE_URL;
        }
      }
    });
  });
...
describe('sendRequestFactory', () => {
  let sendRequest: ReturnType<typeof sendRequestFactory>;
  ...
  describe('Factory Creation', () => {
    it('should create a sendRequest utility with client property', () => {
      expect(sendRequest.client).toBeInstanceOf(HttpClient);
      expect(typeof sendRequest).toBe('function');
    });

    it('should have method-specific functions', () => {
      expect(typeof sendRequest.get).toBe('function');
      expect(typeof sendRequest.post).toBe('function');
      expect(typeof sendRequest.put).toBe('function');
      expect(typeof sendRequest.patch).toBe('function');
      expect(typeof sendRequest.delete).toBe('function');
      expect(typeof sendRequest.head).toBe('function');
      expect(typeof sendRequest.options).toBe('function');
    });
  });
  ...
  describe('Default Handler', () => {
    it('should make GET requests by default', async () => {
      ...
      sendRequest.client.registerApi('test', { baseUrl: 'https://api.example.com' });

      const response = await sendRequest('test', '/users');
  ...
  describe('Method-Specific Functions', () => {
    beforeEach(() => {
      sendRequest.client.registerApi('test', { baseUrl: 'https://api.example.com' });
    });
    ...
    it('should make GET requests', async () => {
      ...
      const response = await sendRequest.get('test', '/users');
    ...
    it('should make POST requests', async () => {
      ...
      const response = await sendRequest.post('test', '/users', { data: postData });
    ...
    it('should make PUT requests', async () => {
      ...
      const response = await sendRequest.put('test', '/users/1', { data: putData });
    ...
    it('should make DELETE requests', async () => {
      ...
      const response = await sendRequest.delete('test', '/users/1');
    ...
    it('should make PATCH requests', async () => {
      ...
      const response = await sendRequest.patch('test', '/users/1', { data: patchData });
    ...
    it('should handle form data with method-specific functions', async () => {
      ...
      const formData = { file: 'test.txt' };
      const response = await sendRequest.post('test', '/upload', { form: formData });
    ...
    it('should handle query parameters with method-specific functions', async () => {
      const mockResponse = { results: [] };
      vi.mocked(fetch).mockResolvedValueOnce({
        ok: true,
        status: 200,
      ...
      const response = await sendRequest.get('test', '/users', { params });
